...
scoreboard players operation #network.transfer_capacity fluid.data = @e[tag=fluid.pipe,predicate=fluid:v1.1/equals_network_id,limit=1] fluid.transfer_capacity

# run
execute if score #network.tank fluid.data matches 0..3 as @e[tag=fluid.tank,predicate=fluid:v1.1/has_network_id,sort=nearest] run function fluid:v1.1/network/transfer_fluid
