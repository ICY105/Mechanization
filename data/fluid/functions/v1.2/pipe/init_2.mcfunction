...
execute if score #pipe.network_id fluid.data matches 1.. as @e[predicate=fluid:v1.2/equals_network_id] run scoreboard players operation @s fluid.network_id = #pipe.network_id fluid.data
execute if score #pipe.network_id fluid.data matches 1.. as @e[type=#fluid:valid_tank_entities,tag=fluid.tank,predicate=fluid:v1.2/has_network_id] run function fluid:v1.2/pipe/update_tank_ids
