...
execute unless score #minecraft.version mechanization.data matches 3105.. run scoreboard players set #install mechanization.data -1
execute if score #install mechanization.data matches -1 run tellraw @a [{"text":"Error: Mechanization v"},{"score":{"name":"#mech.ver.major","objective":"load.status"}},{"text":","},{"score":{"name":"#mech.ver.minor","objective":"load.status"}},{"text":","},{"score":{"name":"#mech.ver.fix","objective":"load.status"}},{"text":" requires Minecraft 1.19.x - Click [here] to download alternate versions.","color":"red","clickEvent":{"action":"open_url","value":"https://github.com/ImCoolYeah105/Mechanization/releases"}}]
