...
execute store result score $temp_0 mech_data run data get entity @s Inventory[{Slot:102b}].tag.mech_upgrades.shield
scoreboard players operation $in_1 mech_data += $temp_0 mech_data

execute store result score $in_0 mech_data run data get entity @s Inventory[{Slot:102b}].tag.Damage
execute if score $in_0 mech_data matches 1.. run item replace block -29999999 0 1601 container.0 from entity @s armor.chest
