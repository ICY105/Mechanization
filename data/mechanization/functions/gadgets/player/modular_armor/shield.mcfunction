...
execute if score @s mechanization.shield < #shield mechanization.data if score #player.out energy.data matches 1.. if data storage du:temp list[].tag.mechanization.upgrades.items[].tag.mech_upgrade{effect:"shield_recharge"} run scoreboard players add @s mechanization.shield 1
