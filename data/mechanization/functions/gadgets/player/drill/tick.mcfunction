...
execute if score #used mechanization.data matches 1.. if data entity @s SelectedItem.tag.mech_upgrades.items[].tag.mechanization.upgrade{effect:"magnetic"} run tp @e[type=item,distance=..10] ~ ~ ~
execute if score #used mechanization.data matches 1.. if data entity @s SelectedItem.tag.mech_upgrades.items[].tag.mechanization.upgrade{effect:"trash"} as @e[type=item,distance=..10] run function mechanization:gadgets/player/drill/trash

execute if score #timer.20 mechanization.data matches 0 if data entity @s SelectedItem.tag.mech_upgrades.items[].tag.mech_upgrade{effect:"aquatic"} if block ~ ~ ~ minecraft:water if block ~ ~1 ~ minecraft:water run effect give @s minecraft:haste 2 0 true
execute if score #timer.20 mechanization.data matches 1 if data entity @s SelectedItem.tag.mech_upgrades.items[].tag.mech_upgrade{effect:"illuminating"} if predicate mechanization:in_darkness run setblock ~ ~ ~ minecraft:light[level=5]
