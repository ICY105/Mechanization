...
execute as @e[type=#mechanization:valid_block_entities,tag=mech_liquid_src,tag=!mech_temp,distance=0.5..1.3] run function mechanization:machines/machines/liquid_pipe/remove_adjacent_pipes_src_2
execute if score $temp_0 mechanization.data matches 1 run tag @s add mech_pipe_input
