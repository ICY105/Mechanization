...
data modify entity @s Item.tag.display.Name set from storage du:temp var

scoreboard players operation @s du_uuid = $uuid.incr du_uuid
...
data modify storage mechanization:networks teleporter[0].Name set from storage du:temp var
