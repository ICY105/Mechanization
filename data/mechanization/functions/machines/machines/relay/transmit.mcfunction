...
execute as @e[distance=1..48,type=minecraft:item_frame,tag=mech_energy_relay,scores={energy.storage=..2000,mechanization.data=1},sort=nearest,limit=1] store success score #success mechanization.data run scoreboard players operation @s energy.storage += #energy mechanization.data
