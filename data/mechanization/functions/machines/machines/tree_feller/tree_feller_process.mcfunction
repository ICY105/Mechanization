...
execute if score #active mechanization.data matches 1 run tp @e[type=item,distance=..64,nbt={Age:0s}] ^ ^-0.5 ^-2
...
# replant
execute if entity @s[tag=mechanization.upgraded,tag=!mechanization.active] if block ~ ~ ~ #mechanization:air as @e[type=item,sort=nearest,limit=1,distance=..12] run function mechanization:machines/machines/tree_feller/replant
