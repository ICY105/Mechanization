...
tag @s remove mechanization.pipe.input

scoreboard players set $temp_0 mechanization.data 0
execute as @e[type=#mechanization:valid_block_entities,tag=mechanization.liquid.send,tag=!mechanization.temp,distance=0.5..1.3] run function mechanization:machines/machines/liquid_pipe/remove_adjacent_pipes_src_2
execute if score $temp_0 mechanization.data matches 1 run tag @s add mechanization.pipe.input
