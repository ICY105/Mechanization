...
execute unless score $minecraft.ver du_data matches 162 run tellraw @a [{"text":"Error: Mechanization v2.3.2b requires Minecraft +1.16.2. Click [here] to download alternate versions.\n","color":"red","clickEvent":{"action":"open_url","value":"https://github.com/ImCoolYeah105/Mechanization/releases"}}]
...
execute if score $install mech_data matches 1 run tellraw @a [{"text":"[Loaded Mechanization v2.3.2b]","color":"dark_green"}]
