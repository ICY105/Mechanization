...
execute unless score $install.ver mech_data matches 16 run tellraw @a [{"text":"Error: Mechanization v2.3.0 requires Minecraft 1.16.x. Click [here] to download alternate versions.\n","color":"red","clickEvent":{"action":"open_url","value":"https://github.com/ImCoolYeah105/Mechanization/releases"}}]
...
execute if score $install mech_data matches 1 run tellraw @a [{"text":"[Loaded Mechanization v2.3.0]","color":"dark_green"}]
