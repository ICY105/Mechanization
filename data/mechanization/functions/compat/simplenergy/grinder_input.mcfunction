...
execute if score #upgrade mechanization.data matches 3 if score #output_count mechanization.data matches 0..62 if data block ~ ~ ~ Items[{Slot:0b}].tag.simplenergy{simplunium_ore:1b} run scoreboard players operation @s mechanization.time = #machines.cf.grinder.speed_ores mechanization.
