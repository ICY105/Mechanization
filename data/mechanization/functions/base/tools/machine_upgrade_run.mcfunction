...
scoreboard players set temp_0 mech_data 0
execute if score in_1 mech_data matches 3 store success score temp_0 mech_data run tag @e[sort=nearest,limit=1,distance=..0.5,tag=!mech_muffler] add mech_muffler
execute if score temp_0 mech_data matches 1 run title @s actionbar [{"translate":"mech.text.muffler_upgrade.use","color":"gold"}]
execute if score temp_0 mech_data matches 1 run clear @s[gamemode=!creative] minecraft:carrot_on_a_stick{mech_itemid:1109} 1
execute if score temp_0 mech_data matches 1 run playsound minecraft:block.wool.place player @s
