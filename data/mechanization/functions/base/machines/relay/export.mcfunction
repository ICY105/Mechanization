...
execute if score $temp_0 mech_data matches 1.. as @e[distance=1..16,type=minecraft:item_frame,tag=mech_energy_relay,sort=nearest,scores={mech_data=2}] unless score @s mech_gridid = $temp_1 mech_data if score @s mech_power < $in_1 mech_data if score $temp_0 mech_data matches 1.. at @s run function mechanization:base/energy/transmit
