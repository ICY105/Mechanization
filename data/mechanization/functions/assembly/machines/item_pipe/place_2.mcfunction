...
execute as @e[tag=mechanization.item_pipe,distance=0.5..1.3] run function mechanization:assembly/machines/item_pipe/place_3
...
execute if score #id mechanization.data matches -1 run scoreboard players operation @s mechanization.item_id = #uuid.incr mechanization.item_id
execute if score #id mechanization.data matches -1 run scoreboard players add #uuid.incr mechanization.item_id.item_id 1
