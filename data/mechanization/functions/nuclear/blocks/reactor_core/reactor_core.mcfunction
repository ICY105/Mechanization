...
scoreboard players operation @s[tag=debug] mechanization.fluid.in -= #flux mechanization.data
execute if score @s mechanization.fluid.in matches ..0 run scoreboard players set @s mechanization.fluid.in 0

execute if entity @s[tag=debug] summon marker run function mechanization:nuclear/blocks/reactor_core/neutron/summon
tag @s remove debug
