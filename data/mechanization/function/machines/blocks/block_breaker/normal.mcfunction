...
execute if score #block_type mechanization.data matches 0..1 if score @s energy.storage >= #machines.cf.block_breaker.power mechanization.data if entity @s[tag=mechanization.upgraded,tag=mechanization.upgraded.ender] run loot spawn ~ ~ ~ mine ~ ~ ~ minecraft:netherite_pickaxe[minecraft:enchantments={"minecraft:silk_touch":1}]
