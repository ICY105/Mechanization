...
execute if entity @s[tag=mechanization.upgraded.nether] if predicate {"condition":"minecraft:random_chance","chance":0.1} run scoreboard players add #storage.2 mechanization.data 576
execute unless data storage mechanization:temp obj.tank_3.tag{id:"molten_nether_alloy"} run data modify entity @s item.components."minecraft:custom_data".fluids[2] set from storage fluid:definitions fluid_data[{id:"molten_nether_alloy"}]
