...
execute as @n[distance=1..48,type=minecraft:item_display,tag=mechanization.energy_relay,scores={energy.storage=..2000,mechanization.data=1}] store success score #success mechanization.data run scoreboard players operation @s energy.storage += #energy mechanization.data
