...
execute if score @s fluid.storage.0 >= #total_cost mechanization.data run item modify block ~ ~ ~ container.12 mechanization:decrement_count
execute if score @s fluid.storage.0 >= #total_cost mechanization.data run scoreboard players operation @s fluid.storage.0 -= #total_cost mechanization.data
execute if score @s fluid.storage.0 >= #total_cost mechanization.data run playsound minecraft:block.enchantment_table.use block @a[distance=..16]
