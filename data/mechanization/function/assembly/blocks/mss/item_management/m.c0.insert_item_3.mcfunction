...
$execute store result score #count.1 mechanization.data run data get storage mechanization:temp obj.drive_contents[$(slot)].components."minecraft:custom_data".mss.count
scoreboard players operation #count.0 mechanization.data += #count.1 mechanization.data

$execute store result storage mechanization:temp obj.drive_contents[$(slot)].components."minecraft:custom_data".mss.count int 1 run scoreboard players get #count.0 mechanization.data 
