...
execute if score #timer.20 mechanization.data matches 5 as @e[type=minecraft:item_display,tag=mechanization.alchemy_chamber] at @s run function mechanization:nuclear/blocks/alchemy_chamber/alchemy_chamber
execute if score #timer.20 mechanization.data matches 6 as @e[type=minecraft:item_display,tag=mechanization.reactor_core] at @s run function mechanization:nuclear/blocks/reactor_core/reactor_core
execute if score #timer.20 mechanization.data matches 7 as @e[type=minecraft:item_display,tag=mechanization.control_rod] at @s run function mechanization:nuclear/blocks/control_rod/control_rod

execute if score #timer.100 mechanization.data matches 10 as @e[type=minecraft:item_display,tag=mechanization.reactor_cell] at @s run function mechanization:nuclear/blocks/reactor_cell/reactor_cell
